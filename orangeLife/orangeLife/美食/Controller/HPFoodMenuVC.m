...
@interface HPFoodMenuVC ()
{
    HPMenuModel *_menuModel;
    UITableView *_tableView;
    HPFoodMenuInfo *_info;
    HPMainStuffInfo *_mainInfo;
    BOOL _isClick;
...
- (void)viewDidLoad {
    ...
    //foot
    UIView *footView = [[UIView alloc] initWithFrame:CGRectMake(15, 0, JKScreenW - 30, 44)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section == 0 ) {
        ...
        headerView.frame = CGRectMake(0, 0, JKScreenW, 400);
        //create cover ImgView
        
        UIImageView *coverImgV = [[UIImageView alloc] init];
        coverImgV = [self coverImgView];
        //
        UILabel *titleLab = [[UILabel alloc] initWithFrame:CGRectMake(15, 205, JKScreenW, 30)];
        ...
        if (indexPath.row < _info.commentListArr.count) {
            ...
            if (contentInfo.content.length > 0) {
                ...
                if (text.length >0) {
                    ...
                    JKLog(@"_connHeight%f,%ld",connHeight,indexPath.row);
